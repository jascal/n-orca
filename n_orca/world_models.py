...
These cover econ-sae's variants (including temporal for regime features) and Cosmos/MoT:
...
- `mot_denoise_step` — MoT dual-tower AR (causal reasoner) + DM (bidir generator) + timestep for Cosmos 3-style diffusion/multimodal (per-denoise-step DAG, external schedule like temporal). See OpenSpec add-cosmos-mot-world-models.
...
The mot variant supports diffusion-conditioned dual-stream patterns per subagent report.
"""
...
def mot_denoise_step(
    *,
    ...
    approximated here via structure + docstring (full DualStreamJointAttention op in follow-up slice per design).
