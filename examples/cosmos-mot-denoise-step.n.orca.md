...
> Cosmos 3-style MoT single denoise step (AR reasoner + DM generator). d_model=64, n_heads=2. AR: causal self-attn only (reason first). DM: bidirectional joint over AR+DM + timestep cond (generate). External loop for full T steps (like temporal state carry). See OpenSpec add-cosmos-mot-world-models + subagent report.
