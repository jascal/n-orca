...
- [ ] 1.3 Update design.md for the chosen approach (reference proposal)

## 2. Implementation

- [ ] 2.1 Add `temporal_world_model` (or `rnn_world_model`) builder in `world_models.py`
- [ ] 2.2 Add corresponding handling in `build_world_model` MCP tool (new variant + kwargs like gru_hidden, etc.)
- [ ] 2.3 Ensure shape inference, verifier, and compilers handle any new state tensors or 3D+ patterns

## 3. Examples & Tests

- [ ] 3.1 Add `examples/econ-temporal-world-model.n.orca.md` + `.mmd` (generated from builder)
- [ ] 3.2 Add test in `test_sae_and_world_models.py` (verifies, compiles, roundtrips)
- [ ] 3.3 Full `n-orca verify` sweep and pytest
