...
- [x] 2.1 Add `mot_denoise_step(...)` (primary) + `mot_reasoner_only(...)` (and optional fuller) builders in `n_orca/world_models.py`. Return Architecture with dual-stream tensors, timestep, appropriate layers/flows for MoT (AR causal path, DM joint path, timestep embed + inject). Docstring references subagent/Cosmos report + "external diffusion schedule like temporal hidden carry". Handle dual-output or main DM out. (Done in scheduler cycle: builder using existing ops per design "smallest slice", + test + ex + verify green; mcp wire 2.3 later. PR#7 review nits addressed post-#6 merge: added approx comment + shape assert in test.)
- [x] 2.2 Add minimal new ops (or extend) in `n_orca/ops/spec.py`: `TimestepEmbed` (or DiffusionTimestep; sinusoidal/learned, pytorch_call), `DualStreamJointAttention` (params for d_model/n_heads; special _torch_call handling concat AR/DM K/V + causal_AR vs bidirectional_DM masks; register). Update any shape inference if needed. (Can start with flows using existing ops + document joint in example if op too heavy for first slice.)
  - TimestepEmbed completed (PR #9 + review feedback addressed).
  - DualStreamJointAttention added (this slice/PR#10: registration + helpers modeled on MHA for now (placeholder impl); used in mot_denoise_step for DM joint path; ex regen; placeholder until full masks/concat in compiler). Both minimal new ops per design. (2.4 etc later)
